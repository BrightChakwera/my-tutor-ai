...
# --- SESSION INITIALIZATION (KeyError Protection) ---
...
if selected_course in active_courses or access_mode == "Premium (Custom Radar)":
    ...
    with tab2:
        ...
        if st.session_state.quiz_set and not st.session_state.quiz_complete:
            ...
            # --- DOWNLOAD SECTION ---
            report_text = f"RADAR GRAD-TUTORS REPORT\nCourse: {selected_course}\nDifficulty: {difficulty}\nScore: {st.session_state.score}/7\nAccuracy: {percent}%"
            st.download_button("📥 Download Scorecard", data=report_text, file_name=f"Radar_{selected_course}_Score.txt")
